    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 7
                curr_is_container, new_is_container = self.ant.container == True, insect.container == True
                curr_is_ant, new_is_ant = self.ant.container == False, insect.container == False
                if curr_is_container and new_is_ant and self.ant.can_contain(insect):
                    self.ant.contain_ant(insect)
                elif curr_is_ant and new_is_container:
                    non_container = self.ant
                    self.ant = insect
                    self.ant.contain_ant(non_container)
                elif (curr_is_container and new_is_container) or (curr_is_ant and new_is_ant):
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 6A
        if self.place.ant == None or self.place.ant.is_ant == True and self.place.ant.blocks_path == False:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        count = 0
        while not place is hive:
            list_of_bees = place.bees
            if list_of_bees:
                if count >= self.min_range and count <= self.max_range:
                    return random_or_none(list_of_bees)
...
class QueenAnt(ThrowerAnt):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.impostor == True:
            ...
            while next_place.exit != None:
                ant = next_place.ant
                if ant != None:
                    if ant.container == False and ant not in QueenAnt.double_damage_ants:
                        ant.damage = ant.damage * 2
                        QueenAnt.double_damage_ants.append(ant)
                    elif ant.container == True:
                            if ant.ant != None:
                                ant_in_container = ant.ant
                            if ant not in QueenAnt.double_damage_ants:
                                ant.damage = ant.damage * 2
                                QueenAnt.double_damage_ants.append(ant)
                            if ant.ant != None and ant_in_container not in QueenAnt.double_damage_ants:
                                ant_in_container.damage = ant_in_container.damage * 2
                                QueenAnt.double_damage_ants.append(ant_in_container)
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 9
        if self.armor - amount <= 0 and self.impostor == False:
            bees_win()
        Insect.reduce_armor(self, amount)
        # END Problem 9
